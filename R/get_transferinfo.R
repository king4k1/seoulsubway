...
get_transferinfo <- function(depart, depart_line, arrival,arrival_line, count = count) {
  ...
  if (count == 1) {
    ...
    transfer_arrival <- checkline(dat = transfer_arrival, 
                                  depart_line = depart_line, arrival_line = arrival_line)
  ...
  if (count == 2) {
    ...
    for (i in 1:nrow(transfer_middle)) {
      transfer_middle_list_sub <- str_remove(transfer_middle$Transfer[i],
                                             fixed(depart_line))
      transfer_middle_list_sub <- unlist(strsplit(transfer_middle_list_sub, split="|", fixed=TRUE))
  ...
  if (count == 3) {
        ...
        for(j2 in 1:nrow(transfer_middle_second)){
          ...
          for(k in seq_along(transfer_middle2_list)){
            ...
            for (k2 in 1:nrow(transfer_middle_get)) {
              transfer_arrival[[paste0(i, "-", j, "-", j2, "-", k, "-", k2)]] <- 
                list(first = transfer_middle_first[i,], second = transfer_middle_second[j2,],
    ...
    for (k in seq_along(transfer_arrival)) {
      ...
      cut_dup[k] <- isTRUE(transfer_arrival[[k]]$second[1, 1] == transfer_arrival[[k]]$third[1, 1])
    }
    # if second transfer station is null(no result) ==> remove one
    cut_ind <- which(cut_crit)
    cut_list <- names(transfer_arrival)[cut_ind]
