...
get_pathresult <- function(shortestpath_result) {
  ...
  if (nrow(Set$Info) == 2) {
    ...
    Total_Depart_Raw <- nrow(subway_data[[as.character(Set$Info[1, 
                                                                "Line"])]])
    Total_Transfer_Raw <- nrow(subway_data[[as.character(Set$Info[2, 
                                                                  "Line"])]])
    ...
    End_Ind2_2 <- which(subway_data[[as.character(Set$Info[2, "Line"])]]$Name == 
                          as.character(Set$Info[2, "Arrive"]))
    Start_Ind3_2 <- which(subway_data[[arrival_line]]$Name == as.character(Set$Info[3, 
                                                                                    "Depart"]))
    ...
    if (isTRUE(as.character(Set$Info[1, "Line"]) == 2) & isTRUE(Set$Info[1, 
                                                                         "Count"] == (Total_Depart_Raw - Start_Ind_2 + End_Ind_2))) {
      Set$Path1 <- subway_data[["2"]][c(Start_Ind_2:Total_Depart_Raw, 
                                        1:End_Ind_2), ]
    } else if (isTRUE(Set$Info[1, "Line"] == 2) & isTRUE(Set$Info[1, 
                                                                  "Count"] == (Total_Depart_Raw - End_Ind_2 + Start_Ind_2))) {
      Set$Path1 <- subway_data[["2"]][c(End_Ind_2:Total_Depart_Raw, 
                                        1:Start_Ind_2), ]
    }
    if (isTRUE(Set$Info[2, "Line"] == 2) & isTRUE(Set$Info[2, "Count"] == 
                                                  (Total_Transfer_Raw - Start_Ind2_2 + End_Ind2_2))) {
      Set$Path2 <- subway_data[["2"]][c(Start_Ind2_2:Total_Transfer_Raw, 
                                        1:End_Ind2_2), ]
    } else if (isTRUE(Set$Info[2, "Line"] == 2) & isTRUE(Set$Info[2, 
    ...
    if (isTRUE(Set$Info[3, "Line"] == 2) & isTRUE(Set$Info[3, "Count"] == 
      ...
      Set$Path3 <- subway_data[["2"]][c(Start_Ind3_2:Total_End_Raw, 
                                        1:End_Ind3_2), ]
    } else if (isTRUE(Set$Info[3, "Line"] == 2) & isTRUE(Set$Info[3, 
                                                                  "Count"] == (Total_End_Raw - End_Ind3_2 + Start_Ind3_2))) {
      Set$Path3 <- subway_data[["2"]][c(End_Ind3_2:Total_End_Raw, 
                                        1:Start_Ind3_2), ]
    }
    ...
    End_Ind3_3 <- which(subway_data[[as.character(Set$Info[3, "Line"])]]$Name == 
                          as.character(Set$Info[3, "Arrive"]))
    Start_Ind4_3 <- which(subway_data[[arrival_line]]$Name == as.character(Set$Info[4, 
                                                                                    "Depart"]))
    End_Ind4_3 <- which(subway_data[[arrival_line]]$Name == as.character(Set$Info[4, 
                                                                                  "Arrive"]))
    Total_Depart_Raw <- nrow(subway_data[[as.character(Set$Info[1, 
                                                                "Line"])]])
    Total_Transfer1_Raw <- nrow(subway_data[[as.character(Set$Info[2, 
                                                                   "Line"])]])
    Total_Transfer2_Raw <- nrow(subway_data[[as.character(Set$Info[3, 
                                                                   "Line"])]])
    ...
    if (isTRUE(Set$Info[1, "Line"] == 2) & isTRUE(Set$Info[1, "Line"] == 
                                                  (Total_Depart_Raw - Start_Ind_3 + End_Ind_3))) {
      Set$Path1 <- subway_data[["2"]][c(Start_Ind_3:Total_Depart_Raw, 
                                        1:End_Ind_3), ]
    } else if (isTRUE(Set$Info[1, "Line"] == 2) & isTRUE(Set$Info[1, 
                                                                  "Line"] == (Total_Depart_Raw - End_Ind_3 + Start_Ind_3))) {
      Set$Path1 <- subway_data[["2"]][c(End_Ind_3:Total_Depart_Raw, 
                                        1:Start_Ind_3), ]
    }
    if (isTRUE(Set$Info[2, "Line"] == 2) & isTRUE(Set$Info[2, "Count"] == 
                                                  (Total_Transfer1_Raw - Start_Ind2_3 + End_Ind2_3))) {
      Set$Path2 <- subway_data[["2"]][c(Start_Ind2_3:Total_Transfer1_Raw, 
                                        1:End_Ind2_3), ]
    } else if (isTRUE(Set$Info[2, "Line"] == 2) & isTRUE(Set$Info[2, 
                                                                  "Count"] == (Total_Transfer1_Raw - End_Ind2_3 + Start_Ind2_3))) {
      Set$Path2 <- subway_data[["2"]][c(End_Ind2_3:Total_Transfer1_Raw, 
                                        1:Start_Ind2_3), ]
    }
    if (isTRUE(Set$Info[3, "Line"] == 2) & isTRUE(Set$Info[3, "Count"] == 
                                                  (Total_Transfer2_Raw - Start_Ind3_3 + End_Ind3_3))) {
      Set$Path3 <- subway_data[["2"]][c(Start_Ind3_3:Total_Transfer2_Raw, 
                                        1:End_Ind3_3), ]
    } else if (isTRUE(Set$Info[3, "Line"] == 2) & isTRUE(Set$Info[3, 
                                                                  "Count"] == (Total_Transfer2_Raw - End_Ind3_3 + Start_Ind3_3))) {
      Set$Path3 <- subway_data[["2"]][c(End_Ind3_3:Total_Transfer2_Raw, 
                                        1:Start_Ind3_3), ]
    }
    if (isTRUE(Set$Info[3, "Line"] == 2) & isTRUE(Set$Info[4, "Count"] == 
                                                  (Total_End_Raw - Start_Ind4_3 + End_Ind4_3))) {
      Set$Path4 <- subway_data[["2"]][c(Start_Ind3_3:Total_End_Raw, 
                                        1:End_Ind4_3), ]
    } else if (isTRUE(Set$Info[4, "Line"] == 2) & isTRUE(Set$Info[4, 
                                                                  "Count"] == (Total_End_Raw - End_Ind4_3 + Start_Ind4_3))) {
      Set$Path4 <- subway_data[["2"]][c(End_Ind4_3:Total_End_Raw, 
                                        1:Start_Ind4_3), ]
    }
