...
shortestpath_2 <- function(depart, depart_line, arrival, arrival_line) {
  ...
  for (i in seq_along(Transfer_List)) {
    ...
    Transfer_First_Ind <- str_remove(Transfer_First$Transfer, depart_line)
    Transfer_First_Ind <- str_split(Transfer_First_Ind, paste0("[", 
                                                               "$", "|", "]"))[[1]]
    Transfer_First_Ind <- Transfer_First_Ind[which(str_detect(Transfer_First_Ind, 
                                                              ""))]
    # process variable for get transfer line
    Transfer_Second_Ind <- str_remove(Transfer_Second$Transfer, depart_line)
    Transfer_Second_Ind <- str_split(Transfer_Second_Ind, paste0("[", 
                                                                 "$", "|", "]"))[[1]]
    Transfer_Second_Ind <- Transfer_Second_Ind[which(str_detect(Transfer_Second_Ind, 
                                                                ""))]
    # process variable for get transfer line
    Transfer_Match_Ind <- which(duplicated(c(Transfer_First_Ind, Transfer_Second_Ind)))
    Transfer_Line <- c(Transfer_First_Ind, Transfer_Second_Ind)[Transfer_Match_Ind]
    # get transfer line for next process
    duplicated_ind <- which(str_detect(Transfer_Line, arrival_line))
    if (isTRUE(length(duplicated_ind) == 0) == FALSE) {
      Transfer_Line <- Transfer_Line[duplicated_ind]
    }
    duplicated_ind2 <- duplicated(Transfer_Line)
    if (length(Transfer_Line) == 2 & isTRUE(sum(duplicated_ind2) >= 
                                            1)) {
      Transfer_Line <- Transfer_Line[duplicated_ind2]
    }
    Total_Transfer_Raw <- nrow(subway_data[[Transfer_Line]])
    Start_Ind2_2 <- which(subway_data[[Transfer_Line]]$Name == Transfer_First$Name)
    for (j in 1:nrow(Transfer_Second)) {
      End_Ind2_2 <- which(subway_data[[Transfer_Line]]$Name == Transfer_Second[j, 
                                                                               "Name"])
      Start_Ind3_2 <- which(subway_data[[arrival_line]]$Name == Transfer_Second[j, 
                                                                                "Name"])
      ...
      Path2_Info <- get_pathinfo(total = Total_Transfer_Raw, ind1 = Start_Ind2_2, 
                                 ind2 = End_Ind2_2, line = Transfer_Line)
      ...
      Transfer_2[[paste0(i, "-", j)]] <- list(Info = data.frame(Depart = c(depart, Transfer_First$Name, Transfer_Second[j, "Name"]),
                                                                ...
                                                                Arrive = c(Transfer_First$Name, Transfer_Second[j, "Name"], arrival)),
