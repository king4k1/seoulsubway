...
shortestpath_1 <- function(departure, departure_line, arrival, arrival_line) {
  ...
  for (i in 1:nrow(Transfer_List)) {
    ...
    Transfer_1[[i]] <- list(Info = data.frame(Departure = c(departure, Transfer_List[i,]$Name),
...
shortestpath_4 <- function(departure, departure_line, arrival, arrival_line) {
  ...
  for (i in seq_along(nn_list_nm)) {
    ...
    for (j in seq_along(multi_linecase)){
      result[[paste0(i, "-", j)]] <- 
        tryCatch(shortestpath_3(departure, departure_line, 
      ...
      result2[[paste0(i, "-", j)]] <- 
        tryCatch(shortestpath_1(departure = nn_list_nm[i], 
                                departure_line = multi_linecase[j],
                                arrival, arrival_line), 
...
shortestpath_all <- function(departure, departure_line, arrival, arrival_line) {
  if (departure_line == arrival_line) {
    Zero <- shortestpath_0(departure = departure, departure_line = departure_line, 
...
shortestpath <- function(departure, arrival) {
  data("subway_data_DT")
  departure_line_list <- subway_data_DT[which(subway_data_DT$Name %in% 
                                             c(departure)), ]$Line
