...
* 이는 분기점의 경우 위경도를 기준으로 산출하였을 시 환승가능한 역이 나오지 않는 경우를 대비하여 argument로 두었습니다.

```{r}
get_transfercriteria(depart="먹골", arrival="혜화", penalty=0.05)
```

#### `get_transferinfo()`는 두 역 사이의 모든 환승가능한 경우를 산출합니다.

```{r, echo=FALSE, out.width=150}
knitr::include_graphics("penalty2.png")
```

* 2번 환승하는 경우 `get_transferinfo()` 함수가 작동하는 원리는 위 그림과 같습니다.
...
#### `shortestpath_0()` 는 depart_line과 arrival_line이 같은 경우 최단거리 경로를 산출합니다..
...
#### `shortestpath_1()` 는 환승을 한번만 하는 경로 중 최단거리 경로를 산출합니다.
...
#### `shortestpath_2()` 는 환승을 두번 하는 경로 중 최단거리 경로를 산출합니다.
