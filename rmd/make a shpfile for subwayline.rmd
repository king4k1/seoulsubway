...
if(Line %in% c("2","6_A")){
  ...
  ll <- cbind(seq(line_data[i,"long"], line_data[i+1,"long"], length = 2000),
              seq(line_data[i,"lat"], line_data[i+1,"lat"], length = 2000))
  Sl <- Line(ll)
  a <- paste0(Line,"_",i)
  ...
  n <- nrow(line_data)
  ll <- cbind(seq(line_data[n,"long"], line_data[1,"long"], length = 2000),
              seq(line_data[n,"lat"], line_data[1,"lat"], length = 2000))
  Sl <- Line(ll)
  a <- paste0(Line,"_",n)
  S1 <- Lines(list(Sl), ID = a)
...
writeOGR(subway_shp, layer = 'subway_shp', "/SNA", driver="ESRI Shapefile")
```
