...
```{r}
seoul0401 <- fread(file=file.choose(), encoding = "UTF-8")
# you need to choose transaction data

seoul_route <- seoul0401%>%select(승차역명, 승차호선명)
...
```{r}
seoul_route[1,]$Name ; seoul_route[1,]$Line ; seoul_route[2,]$Name ; seoul_route[2,]$Line
...
```{r, eval=FALSE}
# if you run this --> more than 24hours... maybe...

subway_route_set <- list()
for(i in 1:277){
  for(j in 1:277){
  ...
  if(i %in% c(10*(1:27),277)){
    save(file="D:/subway_analysis/data/subway_route_set.RData",subway_route_set)
  }
}
