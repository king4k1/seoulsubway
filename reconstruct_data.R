...
reconstruct_data <- function(raw_data, colname_list=c("depart","depart_line",
  ...
  up_Line <- raw_data %>% dplyr::select(colname_list[2]) %>% t() %>% as.vector()
  down_Line <- raw_data %>% dplyr::select(colname_list[4]) %>% t() %>% as.vector()
  up_Name <- raw_data %>% dplyr::select(colname_list[1]) %>% t() %>% as.vector()
  ...
  down_Name <- raw_data %>% dplyr::select(colname_list[3]) %>% t() %>% as.vector()
  ...
  raw_data[,6] <- up_Name
  raw_data[,4] <- up_Line
  raw_data[,9] <- down_Name
  raw_data[,7] <- down_Line
